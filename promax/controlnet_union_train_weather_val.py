...
def build_train_sampler(modality_list, data_root, batch_size, shuffle=True, resolution=256):
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=1, shuffle=False, drop_last=True, num_workers=0) 
        ...
        for step, batch in enumerate(train_dataset):
            with accelerator.accumulate(controlnet):
                ...
                control_image_processor = VaeImageProcessor(
                    vae_scale_factor=vae.config.scaling_factor, do_convert_rgb=True, do_normalize=False
                )
                ...
                # print(f'shape of text_embeds: {batch["unet_added_conditions"]["text_embeds"].shape}')
                # print(f'shape of time_embeds: {batch["unet_added_conditions"]["time_ids"].shape}')
                
                # print(f'text_embeds: {batch["unet_added_conditions"]["text_embeds"]}')
                ...
                # import pdb; pdb.set_trace()
                # added_cond_kwargs = {
                #         "text_embeds": torch.cat([text_embeds, -text_embeds], dim=1).squeeze(0).to(dtype=torch.float32).to(accelerator.device),
                ...
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    ...
                    if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                        image_logs = log_validation(
                            ...
                            step=global_step,
                            val_data_dir=args.train_data_dir,
                        )
