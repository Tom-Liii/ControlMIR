...
custom_module_path = os.path.abspath("/home/heming/research/medical/ControlNetPlus")
...
def main(args):
    ...
    train_dataset, len_train_dataloader = build_train_sampler(modality_list, data_root, args.train_batch_size, shuffle=True)
    ...
    torch.save(modality_embeddings, "/home/heming/research/medical/ControlNetPlus/medical_data/modality_embeddings.pt")
    # exit()
    ...
    controlnet, optimizer, train_dataset, lr_scheduler = accelerator.prepare(
        controlnet, optimizer, train_dataset, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataset):
            with accelerator.accumulate(controlnet):
                ...
                controlnet_image = batch["conditioning_pixel_values"].to(device=accelerator.device, dtype=weight_dtype)
                ...
                if True: # using controlnetplus, hence change controlnet_cond to controlnet_cond_list
                    ...
                    control_type = union_control_type.reshape(1, -1).to(accelerator.device, dtype=batch["prompt_embeds"].dtype).repeat(2, 1)
                    # control_type = control_type.unsqueeze(0).repeat(args.train_batch_size, 1, 1)
                    
                    controlnet_added_cond_kwargs = {
                        "text_embeds": torch.cat([text_embeds, -text_embeds], dim=1).squeeze(0),
                        "time_ids": torch.cat([time_ids, -time_ids], dim=1).squeeze(0),
