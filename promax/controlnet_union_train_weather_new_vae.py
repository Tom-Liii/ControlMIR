...
from evaluation.validate import validate_pipeline
...
def log_validation(vae, unet, controlnet, args, accelerator, weight_dtype, step, is_final_validation=False, val_data_dir=None):
    ...
    validation_images = glob.glob(os.path.join(val_data_dir, "validation", "input", "*.png")) 
    hq_images = [path.replace("input", "target") for path in validation_images]
    validation_prompts = ["Remove Rain"] * len(validation_images)

    for validation_prompt, validation_image, hq_image in tqdm(zip(validation_prompts, validation_images, hq_images), total=len(validation_images), desc="Validating"):
...
def validate(vae_enc, vae_dec, unet, controlnet, dl_val, writer, step, image_dir, args):
    model_gen.eval()
    ...
    num_samples = min(args.val_num, len(dl_val))
    indices = random.sample(range(len(dl_val)), num_samples)
    combined_images = []
    ...
    # MANIQA
    maniqa_metric = pyiqa.create_metric('maniqa').to(device)  # 初始化MANIQA指标

    with torch.no_grad():
        for i, batch in enumerate(tqdm(dl_val, desc="Validation", leave=True)): 
            try:
                x_src = batch["lq"].to(device)
                x_tgt = batch["gt"].to(device)
                
                logging.debug(f"Batch {i}: x_src shape: {x_src.shape}, x_tgt shape: {x_tgt.shape}")
                
                x_tgt_pred = model_gen(x_src, batch=batch, args=args)
                
                logging.debug(f"Batch {i}: x_tgt_pred shape: {x_tgt_pred.shape}")
                
                # 计算无参考指标
                val_nima += nima_metric(x_tgt_pred).mean().item()
                val_musiq += musiq_metric(x_tgt_pred).mean().item()
                val_clip_iqa += metric(x_tgt_pred).mean().item()
                val_liqe += liqe_metric(x_tgt_pred).mean().item()
                val_brisque += brisque_metric(x_tgt_pred).mean().item()
                val_niqe += niqe_metric(x_tgt_pred).mean().item()
                val_maniqa += maniqa_metric(x_tgt_pred).mean().item()  # 计算MANIQA指标
                
                if i in indices:
                    all_image_names.append(batch["image_name"])
                    if x_src.shape[-1] > 1024 or x_src.shape[-2] > 1024:
                        resized_output = resize_transform(x_tgt_pred)
                        resized_gt = resize_transform(x_tgt)
                        resized_src = resize_transform(x_src)
                    else:
                        resized_src = x_src
                        resized_output = x_tgt_pred
                        resized_gt = x_tgt
                    combined_images.append(torch.cat([
                        resized_src,
                        resized_output,
                        resized_gt,
                    ], dim=3))
        ...
        val_nima /= len(dl_val)
        val_musiq /= len(dl_val)
        val_clip_iqa /= len(dl_val)
        val_liqe /= len(dl_val)
        val_brisque /= len(dl_val)
        val_niqe /= len(dl_val)
        val_maniqa /= len(dl_val)  # 平均MANIQA指标
        
        logging.info(f"Validation results - NIMA: {val_nima}, MUSIQ: {val_musiq}, CLIP-IQA: {val_clip_iqa}, LIQE: {val_liqe}, BRISQUE: {val_brisque}, NIQE: {val_niqe}, MANIQA: {val_maniqa}")
        print(f"Validation results - NIMA: {val_nima}, MUSIQ: {val_musiq}, CLIP-IQA: {val_clip_iqa}, LIQE: {val_liqe}, BRISQUE: {val_brisque}, NIQE: {val_niqe}, MANIQA: {val_maniqa}")
        ...
        writer.add_scalar('Validation/MANIQA', val_maniqa, step)  # 记录MANIQA指标
        ...
        for i, combined_image in enumerate(combined_images):
            ...
            print(f"save image: val_pred_{step}_{all_image_names[i]}_{metrics_str}.png")
            combined_image = (combined_image + 1) / 2
            save_image(combined_image, os.path.join(image_dir, f"val_pred_{step}_{all_image_names[i]}_{metrics_str}.png"))
        
    model_gen.train()
    return val_nima, val_musiq, val_clip_iqa, val_liqe, val_brisque, val_niqe, val_maniqa
...
def build_train_sampler(modality_list, data_root, batch_size, shuffle=True, resolution=256):
    dataset = Train_Data(root_dir = data_root, modality_list = modality_list, resolution=resolution)
    val_dataset = Val_Data(root_dir = data_root, modality_list = modality_list, resolution=resolution)
    ...
    val_sampler = DataSampler(val_dataloader)
    # print("data length: \n", dataset.length) 
    
    return sampler, val_sampler, len(dataloader), len(val_dataloader)

def get_train_dataset(args, accelerator, modality_list, data_root, batch_size, shuffle=True):
    ...
    else: # loading medical data
        if args.train_data_dir is not None:
            dataset = Train_Data(root_dir = data_root, modality_list = modality_list)
            val_dataset = Val_Data(root_dir = data_root, modality_list = modality_list)
            # print("data length: \n", dataset.length) 
            # We need to tokenize inputs and targets.
...
def main(args):
    ...
    # Check UNet
    # print("UNet Trainable Parameters (should be LoRA-only):")
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                ...
                load_model = ControlNetModel_Union.from_pretrained(input_dir, subfolder="controlnet")
    ...
    # if args.pretrained_vae_model_name_or_path is not None:
    vae_enc.to(accelerator.device, dtype=torch.float32)
    vae_dec.to(accelerator.device, dtype=torch.float32)
    # else:
    ...
    def compute_embeddings_for_modalities(modality_list, text_encoders, tokenizers, args):
        """Pre-compute embeddings for each modality once"""
        modality_embeddings = {}
        ...
        for modality in modality_list:
            ...
            prompt_embeds, pooled_prompt_embeds = encode_prompt(
                ...
                text_encoders, 
                tokenizers,
                proportion_empty_prompts=0,  # Don't use empty prompts for cached embeddings
                is_train=False
            )
            # import pdb; pdb.set_trace()
            
            # Compute additional embeddings needed for SD XL
            ...
            # Store all embeddings for this modality
    ...
    # modify original dataset loader
    # print(f'------------')
    # print(f'Loading train dataset')
    # print(f'------------')
    data_root = args.train_data_dir
    modality_list = ["rain"] 
    train_dataset, val_dataset, len_train_dataloader, len_val_dataloader = build_train_sampler(modality_list, data_root, args.train_batch_size, shuffle=True, resolution=args.resolution)
    ...
    # print('Saving modality embeddings')
    torch.save(modality_embeddings, "/hpc2hdd/home/sfei285/Project/heming/ControlMIR/medical_data/modality_embeddings.pt")
    # exit()
    ...
    logger.info(f"  Num examples = {len_train_dataloader * args.train_batch_size}")
    logger.info(f"  Num batches each epoch = {len_train_dataloader}")
                ...
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    ...
                    if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                        for tracker in accelerator.trackers:
                            if tracker.name == "tensorboard":
                                with torch.no_grad():
                                    image_logs = validate_pipeline(
                                        vae_enc=vae_enc,
                                        vae_dec=vae_dec,
                                        unet=unet,
                                        controlnet=controlnet,
                                        noise_scheduler=noise_scheduler,
                                        skip_conv_out=skip_conv_out,
                                        val_dataset=val_dataset,  # Note: changed from dl_val to val_dataset to match function signature
